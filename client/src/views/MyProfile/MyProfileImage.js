...
const MyProfileImage = ({photo, previewImage, popupState, main_pic}) => {
	let disabled = false;
	...
	<Container>
		...
		<Row className="justify-content-center" style={{marginBottom: '30px'}}>
				<Button className="btn" disabled={disabled} onClick={() => photoActions('primary', photo.id)} style={{marginRight: '15px'}}>Make photo primary</Button>
				<Button className="btn btn-danger" disabled={disabled} onClick={() => photoActions('delete', photo.id)}>Delete photo</Button>
		</Row>
    </Container>
	);
}
